...
public class SourceReader {
    ...
    private final static LinkedList<String> sourceList = new LinkedList<>(); // stores each line in the file
 ...
 *  @throws IOException if nextLine is null
*/
    public char read() throws IOException {
        if (isPriorEndLine) {            
            ...
            if (nextLine != null) {                
                //System.out.println("READLINE:\t" + nextLine); // adds line to the linked list
                sourceList.add(nextLine);
            }
...
*/
    ...
    public char getNextChar () {
        if (position + 1 >= nextLine.length()) {
            return ' ';
        }
        return nextLine.charAt(position + 1);
    }
 ...
 */
    public static void printFile() {
        for (int lineNum = 0; lineNum < sourceList.size(); lineNum++ ) {
            System.out.println(" " + (lineNum + 1) + ". " + sourceList.get(lineNum));
        }
    }
